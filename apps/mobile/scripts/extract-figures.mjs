 ...
 *
 * Run from the repo root:  node apps/mobile/scripts/extract-figures.mjs
 */
...
function regionsOf(raw) {
  ...
  )].map((m) => ({ d: m[1], transform: m[2], fill: m[3], stroke: m[4], strokeWidth: m[5] }));
}
...
for (const s of all) {
  ...
  const [, label, caption] = m;
  const [muscle, view] = caption.split('·').map((x) => x.trim());
...
for (const s of all) {
  ...
  const regions = regionsOf(s.raw);
  const view = indexOfRegion.front.has(regions[0].d) ? 'front' : 'back';
  const lookup = indexOfRegion[view];
  ...
  strips.push({ key, view, viewBox: s.viewBox, strokeWidth: Number(regions[0].strokeWidth), paint, accent });
}

// --- emit ------------------------------------------------------------------
const q = (v) => JSON.stringify(v);
const lines = [];
lines.push('/* GENERATED by apps/mobile/scripts/extract-figures.mjs — do not edit by hand. */');
